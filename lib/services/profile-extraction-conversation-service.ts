...
async function generateConversationTurn(args: {
  ...
  const result = await executeStructuredGeneration<ConversationTurnModelOutput>({
    ...
    systemInstruction: [
      ...
      "不要用空泛的知识型创作者模板去定义用户。",
      "如果你给出几个选项，它们只能是参考方向，不是让用户只能四选一。",
      "给选项时必须说明用户可以混合多个方向、都不选，或直接用自己的话描述更贴近自己的状态。",
      "优先把选项写成“几个可以参考的思考方向”，不要让问题看起来像标准答案测试。",
      "对表达方式、人设、平台风格这类问题，优先追问用户真实沟通场景、客户反馈和自己最舒服的表达方式。",
      "用户选择某个方向后，下一轮要继续确认哪些部分像他、哪些部分不像他，避免直接定型。",
