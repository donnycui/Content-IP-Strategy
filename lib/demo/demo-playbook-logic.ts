...
function buildStyleContentStep(dashboard: StyleContentDashboardPayload): DemoStep {
  ...
  return {
    ...
    state: dashboard.projects.length ? `已创建内容项目 ${dashboard.projects.length} 个` : "当前还没有内容项目",
    details: [
      buildStateLabel({
...
export function buildDemoPlaybook(input: {
  center: CenterHomePayload;
  styleContent: StyleContentDashboardPayload;
  reviewDashboard: ReviewDashboardPayload;
  evolutionDashboard: EvolutionDashboardPayload;
}): DemoPlaybook {
  return {
    ...
    steps: [
      {
        ...
        href: "/",
        state: input.center.judgment.stageLabel,
        details: [
          `主动作：${input.center.judgment.primaryAction.label}`,
          `次动作：${input.center.judgment.secondaryAction.label}`,
        ],
      },
      {
        title: "IP提炼 Agent",
        ...
        state: input.center.agents.find((item) => item.key === "IP_EXTRACTION")?.status ?? "CURRENT",
        details: [input.center.agents.find((item) => item.key === "IP_EXTRACTION")?.summary ?? "未加载"],
      },
      {
        title: "创作者画像 Agent",
        description: "把提炼结果固化成长期主档，并给后续方向、风格和进化提供共享基线。",
        href: "/agents/creator-profile",
        state: input.center.agents.find((item) => item.key === "CREATOR_PROFILE")?.status ?? "LOCKED",
        details: [input.center.agents.find((item) => item.key === "CREATOR_PROFILE")?.summary ?? "未加载"],
      },
      {
        title: "选题方向 Agent",
        description: "从画像生成方向、主题线和候选题，给内容层提供真正可推进的输入。",
        href: "/agents/topic-direction",
        state: input.center.agents.find((item) => item.key === "TOPIC_DIRECTION")?.status ?? "LOCKED",
        details: [input.center.agents.find((item) => item.key === "TOPIC_DIRECTION")?.summary ?? "未加载"],
      },
      buildStyleContentStep(input.styleContent),
      {
        title: "内容项目总览",
        description: "把所有已创建的内容项目集中起来浏览和进入，作为内容层的统一入口。",
        href: "/content/projects",
        state: input.styleContent.projects.length ? `内容项目 ${input.styleContent.projects.length} 个` : "当前还没有内容项目总览数据",
        details: input.styleContent.projects.slice(0, 3).map((item) => item.project.title),
      },
      buildReviewStep(input.reviewDashboard),
      buildEvolutionStep(input.evolutionDashboard),
      {
        title: "主动学习",
